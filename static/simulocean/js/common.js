...
function new_repo() {
    ...
    var delLink = '<div style="text-align:right;margin-right:10px"><a href="javascript:delIt(' + ct + ')">Delete</a></div>';

    div1.innerHTML = document.getElementById('newrepotpl').innerHTML + delLink;

    document.getElementById('newrepo').appendChild(div1);
...
function delIt(eleId) {
    ...
    var parentEle = d.getElementById('newrepo');
...
function doAction(scriptUrl, scriptPos, scriptArgs, actionDiv) {
    ...
    switch (actVal) {
        ...
        case "deassign":
        case "reports":
        case "uploadToFacility":
        case "updateToFacility":
        case "compile":
        case "execute":
        case "results":
        case "configure":
...
function showMenu(button, scriptPos) {
    ...
    for (var i = 0; i < menuList.length; i++) {
        if (menuList[i] == scriptPos) {
            ...
            if (but.match("add") == "add") {
                but = but.replace(/add/, "minus");
...
function stopUpload(success, inputfile) {
    var result = '';
    if (success == 1) {
    ...
    //document.getElementById('inputfile').innerHTML = '<input type="text" value="hello"></input>';
